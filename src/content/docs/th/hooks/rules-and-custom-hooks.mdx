...
description: 'ทำไม Rules of Hooks ถึงมี (call order), lint plugin ที่บังคับใช้กฎเหล่านี้ และการแยก custom hook มา reuse stateful logic'
sidebar:
  order: 6
---
...
Rules of Hooks — **เรียก hook ที่ top level เท่านั้น เรียกจาก React function เท่านั้น** — มีเพราะ React จับคู่ hook กับ memory slot **ตามลำดับการเรียก** hook แบบมีเงื่อนไขทำให้ slot ที่เหลือทั้งหมดเลื่อนไปอ่าน state ผิด บังคับใช้ด้วย `eslint-plugin-react-hooks` **custom hook** คือแค่ function `use*` ที่เรียก hook อื่น โดยแชร์ **logic ไม่ใช่ state** — ผู้เรียกแต่ละคนได้ state ของตัวเองอิสระ
</Callout>
...
export const quizRules = [
  ...
  {
    q: 'สอง component เรียก `useCounter()` ทั้งคู่ ทั้งสองแชร์อะไรกัน?',
    options: [
