...
`useMemo` cache **value**, `useCallback` cache **function reference** และ `React.memo` ข้าม **re-render ของ child** เมื่อ props ไม่เปลี่ยน — ทั้งหมดเพื่อสู้กับงานที่ไม่จำเป็นจาก reference ใหม่ทุก render ใช้เฉพาะที่ bottleneck จริง ไม่ใช่แบบอัตโนมัติ และด้วย **React Compiler** คุณแทบเลิก memoize เอง: compiler แทรก memoization ที่จุดที่ช่วยให้ หน้าที่คุณจึงเหลือแค่ทำให้ component pure และเขียน code ตรง ๆ
</Callout>
