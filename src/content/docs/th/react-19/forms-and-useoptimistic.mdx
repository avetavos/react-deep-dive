...
**`useFormStatus`** ให้ submit button (ที่ render อยู่ *ข้างใน* `<form>`) อ่าน state `pending` ของ form ได้โดยไม่ต้อง prop drilling **`useOptimistic`** แสดงค่าที่คาดไว้ทันทีและให้ React revert อัตโนมัติถ้า async action ล้มเหลว — ไม่ต้อง roll back เอง เมื่อจับคู่กับ form Action ทำให้ interaction รู้สึกทันที
</Callout>
