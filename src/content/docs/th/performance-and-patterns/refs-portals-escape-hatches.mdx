...
React ให้ **escape hatch** ที่ตั้งใจไว้สำหรับความจำเป็น imperative หายาก: **`createPortal`** render เข้า DOM node อื่น (modal/tooltip) โดยที่ยังอยู่ใน React tree เชิงตรรกะ (context และ event ยังไหลผ่าน); **ref** (+ `useImperativeHandle`) ให้ parent focus/scroll/play แบบ imperative; และ **`flushSync`** บังคับ commit DOM แบบ synchronous เมื่อต้องอ่าน layout ทันที ใช้แต่ละอย่างอย่างประหยัด — อะไรที่ยังเป็น declarative ได้ ควรเป็น
</Callout>
...
export const quizEscape = [
  {
    ...
    options: [
      ...
      'render children เข้า DOM node อื่นโดยที่ยังคงอยู่ใน React tree เชิงตรรกะ',
  ...
  },
  {
    ...
    options: [
      ...
      'บังคับให้ state update และการ commit DOM ของตัวเองเกิดแบบ synchronous — เช่นเพื่ออ่าน layout ใหม่ก่อน paint',
